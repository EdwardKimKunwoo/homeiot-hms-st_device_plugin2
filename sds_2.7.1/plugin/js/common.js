...
class LinearSlider {
	...
	constructor(_parent_, _id_, _start_, _end_, { current, unit, step, updateFN, dlgType, dlgTitle } = {}) {
		...
		this._drawing_attr = {
			...
			handle_margin: undefined,			// Margin between SVG and real handle. 5px.
			slider_handle_margin: undefined,	// Value to move the SVG more for min and max. 10px.
	...
	init() {
		Dom.build(c => {
			this._ele._wrapper = $(c.div( { id: this._id, className: 'iot_LinearSlider wrapper'},
				this._ele._value_wrapper = $(c.div( { className: 'iot_LinearSlider value_wrapper'},
					this._ele._value = $(c.div( { className: 'iot_LinearSlider value'},
		...
		(async () => {
			await appendSvg(this._ele._handler, 'handler_small')
			.then(() => {
		...
		this._ele._handler.bind('touchstart', this._onHandleTouchStart.bind(this));
		this._ele._handler.bind('touchmove', this._onHandleTouchMove.bind(this));
		this._ele._handler.bind('touchend', this._onHandleTouchEnd.bind(this));

		this._ele._slider.bind('click', this._onSliderClick.bind(this));

		if (this._dlgType) {
			this._ele._value.on('click', this._onClickValue.bind(this));
			this._ele._value.setRipple('circle', {size: 40});
		}
		this._ele._handler.setRipple('circle', {size: 20});
	...
	setCurrent(_value_) {
		...
		if (this._drawing_attr.slider_width) {
			this._moveHandle(this._valueToPos(this._current));
		}
	...
	_get_drawing_attr() {
		this._drawing_attr.handle_margin = (this._ele._handler.height() - this._ele._slider.height()) / 2;
		this._drawing_attr.slider_handle_margin = (this._ele._handler.width() / 2 - this._drawing_attr.handle_margin);
	...
	_onHandleTouchMove(_e_) {
		...
		this._moveHandle(_e_.touches[0].clientX);
	}

	_onHandleTouchEnd(_e_) {
		LoggerI("[LinearSlider] Touch End!");
		window.skipRefresh(false);
		this._moveHandle(this._valueToPos(this.__apply_step(this._current)));
	...
	_onSliderClick(_e_) {
		...
		this._prev = this._current;
		this._moveHandle(this._valueToPos(this.__apply_step(this._posToValue(_e_.clientX))));
	...
	_moveHandle(_pos_) {
		var x_pos = _pos_ - this._drawing_attr.slider_offsetX;
		// Block the Moving
		if (x_pos < this._drawing_attr.slider_handle_margin) {
			x_pos = this._drawing_attr.slider_handle_margin;
		} else if (x_pos > this._drawing_attr.slider_width - this._drawing_attr.slider_handle_margin) {
			x_pos = this._drawing_attr.slider_width - this._drawing_attr.slider_handle_margin;
		}

		this._current = this._posToValue(x_pos + this._drawing_attr.slider_offsetX);
		this._ele._text.text(this.__apply_step(this._current) + this._unit);

		// Move Handler
		this._ele._handler.css('transform', ("translateX(" + (x_pos - this._ele._handler.width() / 2) + "px)"));
		// Move Value
		this._ele._value.css('transform', ("translateX(" + (x_pos - this._ele._value.width() / 2) + "px)"));

		// VA
		this._ele._slider_wrapper.attr('aria-valuenow', this._current);
		this._ele._slider_wrapper.attr('aria-valuetext', this._current + this._unit);
	}

	_posToValue(_pos_) {
		return Math.round((this._end - this._start) *
					((_pos_ - (this._drawing_attr.slider_offsetX + this._drawing_attr.slider_handle_margin)) /
					(this._drawing_attr.slider_width - (this._drawing_attr.slider_handle_margin * 2))) + this._start);
    }

    _valueToPos(_value_) {
		return Math.round((_value_ - this._start) *
			((this._drawing_attr.slider_width - (this._drawing_attr.slider_handle_margin * 2)) /
			(this._end - this._start) ) + this._drawing_attr.slider_handle_margin
			+ this._drawing_attr.slider_offsetX);
	}

	__apply_step(value) {
		return ((parseInt(value / this._step) + Math.round((value % this._step) / this._step)) * this._step);
	}
                ...
                let btnArr = {
                    ...
                    [C_("DIALOG_BUTTON_DONE")](dlg, value) {
                        ...
                        if (_that._current !== value) {
							_that.setCurrent(Number(value), true);
							if (_that._updateFN && (_that._current != _that._prev)) {
...
class numberSwiper {
    constructor(_id_, _parent_, _start_, _end_, {_title_, _current_, _update_, _unit_, _setBG_, _enableValueDlg_} = {}) {
        ...
        this._screen = {
            ...
            height: screen.height,
            current_width: screen.width,
			card_gap: undefined,     // Default 26px...
	...
	init() {
		...
		this._screen.card_gap = (this._screen.current_width - this._ele._swiper_container.width());
	...
	_onRotate() {
		...
		this._rotationCheckHandle = setInterval(()=> {
				if ( this._screen.current_width !== screen.width) {
					clearInterval(this._rotationCheckHandle);
					this._rotationCheckHandle = 0;

					this._screen.current_width = screen.width;
					this._left_margin = this._getLeftMargin();
					this._setNumber(this._current, false);
				} else {
	...
	_getLeftMargin() {
		return parseFloat((this._screen.current_width - this._screen.card_gap) - this._number[1].width) / 2 - this._number[1].side_margin;
	}
